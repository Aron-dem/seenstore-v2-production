...
const EXTERNALS = [
  ...
  "@aws-sdk/*", "@azure/*", "@opentelemetry/*", "@google/*",
...
async function buildAll() {
  ...
  // ── 1. Main server bundle (used in dev / Replit) ────────────────────────────
  await esbuild({
    ...SHARED,
  ...
  // ── 2. Vercel serverless handler (exports `app`, no listen) ─────────────────
  await esbuild({
    ...SHARED,
    entryPoints:  { server: path.resolve(artifactDir, "src/vercel-entry.ts") },
    outdir:        apiDir,
    outExtension:  { ".js": ".mjs" },
    sourcemap:     false,
  });
